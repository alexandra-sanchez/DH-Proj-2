...
Drake = read_file("assets/Drake.txt")
View(Drake)
...
g_scores = get_nrc_sentiment(Drake)
class(g_scores)
g_scores
...
g_speech_sen = get_sentences(Drake)
g_speech_sen
...
## if one is absent then go to the console below and type install.packages("devtools")
library("devtools")
...
wordcloud(Drake,colors=c("blue","green"), max.words = 25)
